...
export class EventTracker {
  ...
  /** Optional gate: returns false when daemon is not connected, suppressing all event tracking. */
  private connectionGate: () => boolean = () => true;
  ...
  /**
   * Set a connection gate function. When the gate returns false, all event
   * tracking is suppressed. This prevents wasted IPC calls when the daemon
   * is not running (e.g., during bootstrap or after disconnect).
   */
  public setConnectionGate(gate: () => boolean): void {
  ...
  private onFileOpened(document: vscode.TextDocument): void {
    if (!this.enabled || !this.connectionGate()) {
      return;
    }
